...
export async function getZApiGroups() {
    ...
    const response = await fetch(`https://api.z-api.io/instances/${instanceId}/token/${token}/groups?page=1&pageSize=200`, {
        headers: {
            "Content-Type": "application/json",
            "Client-Token": clientToken
        }
    ...
    if (!response.ok) {
        const error = await response.text();
        throw new Error(`Erro ao buscar grupos Z-API: ${error}`);
    }
